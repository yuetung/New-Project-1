...
namespace UniRPGEditor
{
	public class FindMissingScripts : EditorWindow 
	{
					...
					foreach (Info nfo in r)
					{
						EditorGUILayout.BeginHorizontal();
						{
							if (GUILayout.Button("*", GUILayout.Width(25)))
							{
								...
								{
									if (UniRPGEditorGlobal.SaveCurrentSceneIfUserWantsTo())
									{
										if (UniRPGEditorGlobal.OpenScene(nfo.scene))
										{
		...
		{
			string currScene = UniRPGEditorGlobal.CurrentScene();
			...
			for (int i = 0; i < files.Length; i++)
			{
				...
				if (!string.IsNullOrEmpty(fn))
				{
					if (UniRPGEditorGlobal.OpenScene(fn))
					{
			...
			if (string.IsNullOrEmpty(currScene)) UniRPGEditorGlobal.NewScene();
			else UniRPGEditorGlobal.OpenScene(currScene);
