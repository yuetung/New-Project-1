...
#else
	private static List<AsyncOperation> loadingLevels = new List<AsyncOperation>();
	public static bool isLoadingLevel
	{
		get
		{
			...
			{
				for (int i = loadingLevels.Count - 1; i >= 0; i--)
				{
					if (loadingLevels[i].isDone) loadingLevels.RemoveAt(i);
				}

				if (loadingLevels.Count > 0) return true;
			}
			return false;
		}
	}
